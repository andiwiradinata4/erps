Public Class frmMstBusinessPartnerBankAccount
    ...
    Private Sub prvQuery()
        Try
            grdMain.DataSource = BL.BusinessPartner.ListDataBankAccount(pubBPID)
    ...
    Public Sub pubRefresh(Optional ByVal strSearch As String = "")
        With grdView
            If Not grdView.FocusedValue Is Nothing And strSearch = "" Then
                strSearch = grdView.GetDataRow(grdView.FocusedRowHandle).Item("AccountName")
            End If
            prvQuery()
            If grdView.RowCount > 0 Then UI.usForm.GridMoveRow(grdView, "AccountName", strSearch)
        End With
    End Sub
    ...
    Private Sub prvNew()
        Dim frmDetail As New frmMstBusinessPartnerBankAccountDet
        With frmDetail
            .pubIsNew = True
            .pubBPID = pubBPID
    ...
    Private Sub prvUserAccess()
        With ToolBar.Buttons
            .Item(cNew).Visible = BL.UserAccess.IsCanAccess(ERPSLib.UI.usUserApp.UserID, ERPSLib.UI.usUserApp.ProgramID, VO.Modules.Value.MasterBusinessPartnerBankAccount, VO.Access.Value.NewAccess)
            .Item(cDelete).Visible = BL.UserAccess.IsCanAccess(ERPSLib.UI.usUserApp.UserID, ERPSLib.UI.usUserApp.ProgramID, VO.Modules.Value.MasterBusinessPartnerBankAccount, VO.Access.Value.DeleteAccess)
        End With
    End Sub

#Region "Form Handle"

    Private Sub frmMstBusinessPartnerBankAccount_KeyDown(sender As Object, e As KeyEventArgs) Handles MyBase.KeyDown
        If e.KeyCode = Keys.Escape Then
            If UI.usForm.frmAskQuestion("Tutup form?") Then Me.Close()
        End If
    End Sub

    Private Sub frmMstBusinessPartnerBankAccount_Load(sender As Object, e As EventArgs) Handles MyBase.Load
