...
Public Class frmTraConfirmationClaim

    Private intPos As Integer = 0
    Private clsData As New VO.ConfirmationClaim
    ...
    Private Sub prvQuery()
        ...
        pgMain.Value = 30
        Try
            dtData = BL.ConfirmationClaim.ListData(intProgramID, intCompanyID, dtpDateFrom.Value.Date, dtpDateTo.Value.Date, cboStatus.SelectedValue, intClaimType)
    ...
    Private Function prvGetData() As VO.ConfirmationClaim
        Dim clsReturn As New VO.ConfirmationClaim
        ...
        clsReturn.ConfirmationClaimNumber = grdView.GetRowCellValue(intPos, "ClaimNumber")
        clsReturn.ConfirmationClaimDate = grdView.GetRowCellValue(intPos, "ClaimDate")
        ...
        clsReturn.ClaimID = grdView.GetRowCellValue(intPos, "ClaimID")
    ...
    Private Sub prvNew()
        prvResetProgressBar()
        Dim frmDetail As New frmTraConfirmationClaimDet
        With frmDetail
            .pubIsNew = True
            .pubCS = prvGetCS()
    ...
    Private Sub prvDelete()
        ...
        If Not UI.usForm.frmAskQuestion("Hapus Nomor " & clsData.ConfirmationClaimNumber & "?") Then Exit Sub
        ...
        pgMain.Value = 40
        Try
            BL.ConfirmationClaim.DeleteData(clsData.ID, clsData.Remarks)
            pgMain.Value = 100
            UI.usForm.frmMessageBox("Hapus data berhasil.")
            pubRefresh(grdView.GetRowCellValue(intPos, "ConfirmationClaimNumber"))
        Catch ex As Exception
            UI.usForm.frmMessageBox(ex.Message)
        Finally
            Me.Cursor = Cursors.Default
    ...
    Private Sub prvSubmit()
        ...
        clsData = prvGetData()
        clsData.LogBy = ERPSLib.UI.usUserApp.UserID
        If Not UI.usForm.frmAskQuestion("Submit Nomor " & clsData.ConfirmationClaimNumber & "?") Then Exit Sub

        Me.Cursor = Cursors.WaitCursor
        pgMain.Value = 40
        Try
            BL.ConfirmationClaim.Submit(clsData.ID, "")
            pgMain.Value = 100
    ...
    Private Sub prvCancelSubmit()
        ...
        clsData = prvGetData()
        clsData.LogBy = ERPSLib.UI.usUserApp.UserID
        If Not UI.usForm.frmAskQuestion("Batal Submit Nomor " & clsData.ConfirmationClaimNumber & "?") Then Exit Sub
        ...
        pgMain.Value = 40
        Try
            BL.ConfirmationClaim.Unsubmit(clsData.ID, clsData.Remarks)
            pgMain.Value = 100
    ...
    Private Sub prvPrint()
        ...
        'clsData = prvGetData()
        'Try
        '    Dim dtData As DataTable = BL.ConfirmationClaim.PrintVer00(clsData.ProgramID, intCompanyID, strID)
...
#Region "Form Handle"
    ...
    Private Sub frmTraConfirmationClaim_KeyDown(sender As Object, e As KeyEventArgs) Handles MyBase.KeyDown
        If e.KeyCode = Keys.Escape Then
            If UI.usForm.frmAskQuestion("Tutup form?") Then Me.Close()
        End If
    End Sub

    Private Sub frmTraConfirmationClaim_Load(sender As Object, e As EventArgs) Handles MyBase.Load
