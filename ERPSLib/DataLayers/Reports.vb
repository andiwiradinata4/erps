...
"	AND PC.PCDate>=@DateFrom AND PC.PCDate<=@DateTo" & vbNewLine
        ...
        Public Shared Function MonitoringProductTransactionReportSalesContractARVer00(ByRef sqlCon As SqlConnection, ByRef sqlTrans As SqlTransaction,
            ...
            With sqlcmdExecute
                ...
                .CommandText =
"SELECT   " & vbNewLine &
"	ROW_NUMBER() OVER(ORDER BY PCD.ID) AS ID, PCD.ID AS PCDetailID, SC.SCNumber, SC.SCDate, SC.BPName, SC.ARNumber, SC.ARDate, PCD.OrderNumberSupplier, " & vbNewLine &
"	CASE WHEN SC.ItemCodeExternal='' THEN PCD.OrderNumberSupplier ELSE SC.ItemCodeExternal END ItemCodeExternal, SC.TotalWeight    " & vbNewLine &
"FROM traPurchaseContract PC   " & vbNewLine &
"INNER JOIN traPurchaseContractDet PCD ON   " & vbNewLine &
"	PC.ID=PCD.PCID   " & vbNewLine &
"	AND PC.IsDeleted=0   " & vbNewLine &
...
"		SCDCO.CODetailID, SCH.SCNumber, SCH.SCDate, BP.Name AS BPName, SCD.OrderNumberSupplier, ARD.ARNumber, ARD.ARDate, ARD.ItemCodeExternal, ARD.TotalWeight    " & vbNewLine &
"	FROM traSalesContract SCH   " & vbNewLine &
"	INNER JOIN mstBusinessPartner BP ON   " & vbNewLine &
"		SCH.BPID=BP.ID   " & vbNewLine &
"	INNER JOIN traSalesContractDet SCD ON   " & vbNewLine &
...
"	(  " & vbNewLine &
"		SELECT  " & vbNewLine &
"			ARH.ARDate, ARH.ARNumber, ARI.ReferencesDetailID, MI.ItemCodeExternal, SUM(ARI.TotalWeight) TotalWeight " & vbNewLine &
...
"		GROUP BY ARH.ARDate, ARH.ARNumber, ARI.ReferencesDetailID, MI.ItemCodeExternal " & vbNewLine &
" " & vbNewLine &
"		UNION ALL  " & vbNewLine &
"		SELECT  " & vbNewLine &
"			ARH.ARDate, ARH.ARNumber, ARI.ReferencesParentID AS ReferencesDetailID, MI.ItemCodeExternal, SUM(ARI.TotalWeight) TotalWeight " & vbNewLine &
"		FROM traAccountReceivable ARH  " & vbNewLine &
"		INNER JOIN traARAPItem ARI ON  " & vbNewLine &
"			ARH.ID=ARI.ParentID  " & vbNewLine &
"			AND ARI.ReferencesParentID<>''  " & vbNewLine &
...
"			ARH.IsDeleted=0  " & vbNewLine &
"			AND ARH.ApprovedBy<>''  " & vbNewLine &
"		GROUP BY ARH.ARDate, ARH.ARNumber, ARI.ReferencesParentID, MI.ItemCodeExternal " & vbNewLine &
"	) ARD ON   " & vbNewLine &
