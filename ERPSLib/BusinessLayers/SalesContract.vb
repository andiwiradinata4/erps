...
#Region "Main"
        ...
        Public Shared Function ListDataDifferentParentWithTotalChild(ByVal intProgramID As Integer, ByVal intCompanyID As Integer,
                                                                     ByVal dtmDateFrom As DateTime, ByVal dtmDateTo As DateTime) As DataTable
            BL.Server.ServerDefault()
            Using sqlCon As SqlConnection = DL.SQL.OpenConnection
                Return DL.SalesContract.ListDataDifferentParentWithTotalChild(sqlCon, Nothing, intProgramID, intCompanyID, dtmDateFrom, dtmDateTo)
            End Using
        End Function

        Public Shared Function ListDataDifferentParentWithTotalChildKO(ByVal intProgramID As Integer, ByVal intCompanyID As Integer,
                                                                       ByVal dtmDateFrom As DateTime, ByVal dtmDateTo As DateTime) As DataTable
            BL.Server.ServerDefault()
            Using sqlCon As SqlConnection = DL.SQL.OpenConnection
                Return DL.SalesContract.ListDataDifferentParentWithTotalChildKO(sqlCon, Nothing, intProgramID, intCompanyID, dtmDateFrom, dtmDateTo)
            End Using
        End Function
...
#Region "Detail Confirmation Order"
        ...
        Public Shared Function SaveDataDetailCOSubitem(ByVal bolNew As Boolean, ByVal strSCID As String,
            ...
            Using sqlCon As SqlConnection = DL.SQL.OpenConnection
                Dim sqlTrans As SqlTransaction = sqlCon.BeginTransaction
                Try
                    bolReturn = SaveDataDetailCOSubitem(sqlCon, sqlTrans, bolNew, strSCID, clsData)
                    sqlTrans.Commit()
                Catch ex As Exception
                    sqlTrans.Rollback()
                    Throw ex
        ...
        Public Shared Function SaveDataDetailCOSubitemMultiple(ByVal strSCID As String, ByVal clsDataAll As List(Of VO.SalesContractDetConfirmationOrder)) As Boolean
            Dim bolReturn As Boolean = False
            BL.Server.ServerDefault()
            Using sqlCon As SqlConnection = DL.SQL.OpenConnection
                Dim sqlTrans As SqlTransaction = sqlCon.BeginTransaction
                Try
                    For Each clsData As VO.SalesContractDetConfirmationOrder In clsDataAll
                        SaveDataDetailCOSubitem(sqlCon, sqlTrans, True, strSCID, clsData)
                    Next
                    sqlTrans.Commit()
                    bolReturn = True
                Catch ex As Exception
        ...
        Public Shared Function SaveDataDetailCOSubitem(ByRef sqlCon As SqlConnection, ByRef sqlTrans As SqlTransaction,
                                                       ByVal bolNew As Boolean, ByVal strSCID As String,
                                                       ByVal clsData As VO.SalesContractDetConfirmationOrder) As Boolean
            Dim bolReturn As Boolean = False
            Try
                If bolNew Then
