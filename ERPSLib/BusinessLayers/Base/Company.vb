    ...
    Public Class Company
        ...
        Public Shared Function ListData() As DataTable
            ...
            Using sqlCon As SqlConnection = DL.SQL.OpenConnection
                Return DL.Company.ListData(sqlCon, Nothing)
            End Using
        End Function
        ...
        Public Shared Function ListDataForChooseDefault() As DataTable
            BL.Server.ServerDefault()
            Using sqlCon As SqlConnection = DL.SQL.OpenConnection
                Return DL.Company.ListDataForChooseDefault(sqlCon, Nothing)
            End Using
        End Function
        ...
        Public Shared Function SaveData(ByVal bolNew As Boolean, ByVal clsData As VO.Company) As Integer
            ...
            Try
                Using sqlCon As SqlConnection = DL.SQL.OpenConnection
                    If bolNew Then
                        clsData.ID = DL.Company.GetMaxID(sqlCon, Nothing)
                        If DL.Company.DataExists(sqlCon, Nothing, clsData.ID) Then
                            Err.Raise(515, "", "ID sudah ada sebelumnya")
                        End If
                    End If

                    If DL.Company.InitialExists(sqlCon, Nothing, clsData.CompanyInitial, clsData.ID) Then
                        Err.Raise(515, "", "Inisial perusahaan sudah ada sebelumnya")
                    End If

                    DL.Company.SaveData(sqlCon, Nothing, bolNew, clsData)
                End Using
        ...
        Public Shared Function GetDetail(ByVal intID As Integer) As VO.Company
            BL.Server.ServerDefault()
            Using sqlCon As SqlConnection = DL.SQL.OpenConnection
                Return DL.Company.GetDetail(sqlCon, Nothing, intID)
            End Using
        End Function
        ...
        Public Shared Sub DeleteData(ByVal intID As Integer)
            ...
            Try
                Using sqlCon As SqlConnection = DL.SQL.OpenConnection
                    If DL.Company.GetStatusID(sqlCon, Nothing, intID) = VO.Status.Values.InActive Then
                        Err.Raise(515, "", "Data tidak dapat dihapus. Dikarenakan data telah tidak aktif")
                    Else
                        DL.Company.DeleteData(sqlCon, Nothing, intID)
                    End If
                End Using
            Catch ex As Exception
                Throw ex
