        ...
        Public Shared Function SaveDataVer02_ReceivePayment(ByRef sqlCon As SqlConnection, ByRef sqlTrans As SqlTransaction,
            ...
            Try
                ...
                ElseIf clsData.Modules.Trim = VO.AccountReceivable.ReceivePaymentClaimPOCutting Then
                    dtReferencesItem = DL.Cutting.ListDataDetailResult(sqlCon, sqlTrans, clsData.ReferencesID)
                ...
                Else
                    ...
                    For Each dr As DataRow In dtDetailItem.Rows
                        ...
                        If clsData.Modules.Trim = VO.AccountReceivable.ReceivePaymentClaimPOCutting Then DL.Cutting.CalculateTotalUsedReceiveItemPaymentClaimVer02(sqlCon, sqlTrans, dr.Item("ReferencesDetailID"))
                    ...
                    ElseIf clsData.Modules.Trim = VO.AccountReceivable.ReceivePaymentSalesReturn Then
                        '# Calculate Sales Return Header
                        DL.SalesReturn.CalculateTotalUsedReceivePaymentVer02(sqlCon, sqlTrans, clsData.ReferencesID)
                    ElseIf clsData.Modules.Trim = VO.AccountReceivable.ReceivePaymentClaimPOCutting Then
                        '# Calculate Claim PO Cutting
                        DL.Cutting.CalculateTotalUsedReceivePaymentClaimVer02(sqlCon, sqlTrans, clsData.ReferencesID)
                    ElseIf clsData.Modules.Trim = VO.AccountReceivable.ReceivePaymentClaimPurchase Then
                ...
                For Each clsItem As VO.ARAPItem In clsData.ARAPItem
                    ...
                    If clsData.Modules.Trim = VO.AccountReceivable.ReceivePaymentClaimPOCutting Then DL.Cutting.CalculateTotalUsedReceiveItemPaymentClaimVer02(sqlCon, sqlTrans, clsItem.ReferencesDetailID)
                ...
                If clsData.Modules.Trim = VO.AccountReceivable.ReceivePaymentClaimPOCutting Then DL.Cutting.CalculateTotalUsedReceivePaymentClaimVer02(sqlCon, sqlTrans, clsData.ReferencesID)
        ...
        Public Shared Sub DeleteDataVer02(ByRef sqlCon As SqlConnection, ByRef sqlTrans As SqlTransaction,
            ...
            Try
                ...
                For Each dr As DataRow In dtDetailItem.Rows
                    ...
                    If strModules.Trim = VO.AccountReceivable.ReceivePaymentClaimPOCutting Then DL.Cutting.CalculateTotalUsedReceiveItemPaymentClaimVer02(sqlCon, sqlTrans, dr.Item("ReferencesDetailID"))
                ...
                For Each dr As DataRow In dtDetail.Rows
                    ...
                    ElseIf strModules.Trim = VO.AccountReceivable.ReceivePaymentSalesReturn Then
                    ...
                    ElseIf strModules.Trim = VO.AccountReceivable.ReceivePaymentClaimPOCutting Then
                        DL.Cutting.CalculateTotalUsedReceivePaymentClaimVer02(sqlCon, sqlTrans, dr.Item("InvoiceID"))
                ...
                ElseIf strModules.Trim = VO.AccountReceivable.ReceivePaymentSalesReturn Then
                    ...
                    DL.SalesReturn.CalculateTotalUsedReceivePaymentVer02(sqlCon, sqlTrans, clsExists.ReferencesID)

                ElseIf strModules.Trim = VO.AccountReceivable.ReceivePaymentClaimPOCutting Then

                    '# Calculate Claim PO Cutting
                    DL.Cutting.CalculateTotalUsedReceivePaymentClaimVer02(sqlCon, sqlTrans, clsExists.ReferencesID)
        ...
        Public Shared Sub GenerateJournal(ByRef sqlCon As SqlConnection, ByRef sqlTrans As SqlTransaction,
            ...
            Try
                ...
                If clsData.IsDP Then '# Pembayaran DP
                    ...
                    End If
                Else
                    '# Handle Sales Return | Revert Sales / Delivery Journal
                    If clsData.Modules = VO.AccountReceivable.ReceivePaymentSalesReturn Then

                        '# Akun Piutang Belum Ditagih -> Debit
                        clsJournalDetail.Add(New VO.JournalDet With
                                             {
                        ...
                        If clsData.DPAmount > 0 Then
                            ...
                            '# Akun Uang Muka Penjualan -> Kredit
                            clsJournalDetail.Add(New VO.JournalDet With
                                         {
                                             .CoAID = ERPSLib.UI.usUserApp.JournalPost.CoAofPrepaidIncome,
                                             .DebitAmount = 0,
...
EndProcess:
        ...
        Public Shared Sub GenerateJournalInvoice(ByRef sqlCon As SqlConnection, ByRef sqlTrans As SqlTransaction,
            ...
            Try
                ...
                If clsData.Modules.Trim = VO.AccountReceivable.ReceivePaymentSalesReturn Then
                    '# Akun Piutang Usaha -> Debit u/Kebalikan dari penjualan / pengiriman
                    clsJournalDetail.Add(New VO.JournalDet With
                                         {
                                             .CoAID = ERPSLib.UI.usUserApp.JournalPost.CoAofAccountReceivable,
                    ...
                    '# Akun Kas / Bank -> Kredit u/Kebalikan dari penjualan / pengiriman
                    clsJournalDetail.Add(New VO.JournalDet With
                                         {
                                             .CoAID = clsData.CoAIDOfIncomePayment,
                                             .DebitAmount = 0,
                    ...
                    '# Setup Akun PPH u/Kebalikan dari penjualan / pengiriman
                    If clsData.TotalPPH > 0 Then
                        intGroupID += 1
...
#Region "Detail"
        ...
        Public Shared Function ListDataDetailItemReceiveWithOutstandingVer02(ByVal intCompanyID As Integer, ByVal intProgramID As Integer,
            ...
            BL.Server.ServerDefault()
            Using sqlCon As SqlConnection = DL.SQL.OpenConnection
                Return DL.AccountReceivable.ListDataDetailItemReceiveWithOutstandingVer02(sqlCon, Nothing, intCompanyID, intProgramID, intBPID, strAPID)
            End Using
        End Function
